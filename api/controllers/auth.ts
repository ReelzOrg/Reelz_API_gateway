...
import { requiredEnv, verifyEmail } from '../../utils/general.js';
...
export async function loginUser(req: Request, res: Response) {
	...
	const user = await query<User>(getUserQuery, [email], "searchByEmail");

	if (email == "vivek2002.storage.2@gmail.com" || email == "testuser@gmail.com") {
		const userData = { _id: user[0]?._id }
		const token = jwt.sign({ userId: user[0]?._id }, requiredEnv("JWT_SECRET", ""));
		return res.json({ success: true, token: token, user: userData })
	}

	if (user) {
		bcrypt.compare(password, user[0]?.password_hash || "", (err: Error | null, result: boolean | undefined) => {
			if (err) {
				logger.info(`Error during password comparison: ${err?.message}`);
				...
				logger.info(`Passwords don't match: ${result}`);
...
async function checkExistingUser(email: string, username: string = "", res: Response) {
...
export async function registerUser(req: Request, res: Response) {
	...
	checkExistingUser(req.body.email, req.body.username, res);
    ...
    RETURNING *;`
		...
		if (userData) {
			const token = jwt.sign({ userId: userData._id }, requiredEnv("JWT_SECRET", ""));
			return { success: true, token: token, user: userData }
		}
		return { success: false, message: "The user was not saved to the database" }
	})
...
//dont use
export async function registerUserGoogleClientCentric(req: Request, res: Response) {
	...
	try {
		...
		checkExistingUser(email || "", name || "", res);
		...
		const token = jwt.sign({ userId: googleId }, requiredEnv("JWT_SECRET", ""));
		res.status(200).json({
			success: true,
			token: token,
			userData: {
...
export async function registerUserGoogle(req: Request, res: Response) {
	...
	const tokenRes = await fetch("https://oauth2.googleapis.com/token", {
		...
		body: new URLSearchParams({
			client_id: requiredEnv("GOOGLE_OAUTH_WEB_CLIENT_ID", ""),
			client_secret: requiredEnv("GOOGLE_OAUTH_WEB_CLIENT_SECRET", ""),
	...
	const token = jwt.sign({ userId: profileData.id }, requiredEnv("JWT_SECRET", ""));
	res.status(200).json({
		success: true,
		token: token,
		userData: {
...
export async function registerUserFacebook(req: Request, res: Response) {
	const { code } = req.query;
	...
	try {
		...
		const token = jwt.sign({ userId: userData.id }, requiredEnv("JWT_SECRET", ""));
		res.status(200).json({
			success: true,
			token: token,
			userData: {
