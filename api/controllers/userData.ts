...
export async function getUserBasicData(req: Request, res: Response) {
...
export async function getUserProfile(req: Request, res: Response) {
	...
	const limit = 9;
	const reqUserOffset: number = page * limit;

	const userProfileQuery = `SELECT * FROM users WHERE username = $1 LIMIT 1;`;
	const requestedUser: User[] = await query(userProfileQuery, [reqUserUsername], "getUserByUsername");
	...
	if (requestedUser) {
		let userData: User = {
			...
			dob: requestedUser[0]?.dob,
			gender: requestedUser[0]?.gender,
...
export async function getUserFeed(req: Request, res: Response) {
	...
	const topLimit = totalFollows.length === 0 ? 0 : Math.ceil(Number(totalFollows[0]?.get('totalFollows')) * 0.1);
...
LIMIT 20`
	...
	let viewedPostIds: { post_id: string }[] = await query<{ post_id: string }>(getViewedPostIdsQuery, [loggedInUser], "getViewedPostIds");
	let viewedPostIdList = viewedPostIds.map((postId) => postId.post_id)
	...
	const userFeedPostIds = userFeed.map((post) =>
		...
		.filter((item) => !viewedPostIdList.includes(item[0]));
...
export async function handleFollow(req: Request, res: Response) {
	...
	const userisPrivate = await query<{ is_private: boolean }>(isUserPrivateQuery, [reqUserId], "isUserPrivate");
...
export async function getNetworkList(req: Request, res: Response, networkType: string) {
	const reqUserId = req.params.id;
	const loggedinUserId = req.user?.userId;
	let getNetworkListQuery: string = `MATCH (user:User {_id: $loggedInUser})-[:FOLLOWS]->(f:User) RETURN COLLECT(f) AS fu;`;
	const limit = 20;
	...
	if (networkType == "followers") {
		getNetworkListQuery = `MATCH (user:User {_id: $loggedInUser})<-[:FOLLOWS]-(f:User) RETURN COLLECT(f) AS fu;`;
	}
...
RETURN collect(follower.userId) AS orderedFollowerIds
		...
		const userIds = following.map((userNode: any) => userNode.properties._id);
	...
	const userisPrivate = await query<{ is_private: boolean }>(getUserByIdQuery, [reqUserId], "getuserById");
	...
	networkType == "followers"
		...
		? res.json({ success: true, network: networkList[0]?.get("fu") })
		: res.json({ success: true, network: networkList[0]?.get("fu") });
	return;
}
...
export async function handlePostUpload(req: Request, res: Response) {
	...
	const mimeType = typeof req.body.fileType == "string" ? req.body.fileType.split("/")[0] : req.body.fileType.map((file: string) => file.split("/")[0]);
	...
	const post: any = await query(postQuery, [loggedInUserId, req.body.caption, mediaUrl, mimeType], "createPost");
	...
	const neo4jPost: any = await neo4jQuery(createPostQuery, { userId: loggedInUserId, postId: post[0].post_id }, "createPost");

	return { success: true, message: "Post uploaded successfully", post: post }
}

// /:id/save-viewed-posts
export async function saveViewedPosts(req: Request, res: Response) {
...
// /:id/process-media
export async function sendProcessingRequest(req: Request, res: Response) {
	/**
	 * toProcessUrls: [{
	 * 	"url": "https://reelzapp.s3.us-east-1.amazonaws.com/userPosts/44938b73-afca-4cf9-a298-bd6c5a7bda46/97f5cd4d-e515-4775-b7b6-baf649f6c5c5/post_1000000046-0",
	 * 	"mediaType": "image",
	 * 	"dimensions": {
	 * 		"width": 100,
	 * 		"height": 100
	...
	//TODO: if the uploadType is post and there is only 1 video file, then maybe convert the uploadType to reel
	const { toProcessUrls, uploadType, post_id } = req.body;
	const processingRequestData = {
		value: {
			toProcessUrls, uploadType, post_id, timeStamp: Date.now()
		},
		headers: {
			'x-request-id': (req.id).toString() || 'no-request-id',
	...
	try {
		...
		const result = await KafkaProducerManager.sendAvro(
			ProducerNames.MEDIA,
