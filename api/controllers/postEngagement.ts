...
export async function likeOnPost(req: Request, res: Response) {
	const loggedInUserId = req.user?.userId;
	const postId = req.params.id;

	//DONT HAVE TO CHECK IF THE LIKE WAS MADE BECAUSE WE HAVE A CONSTRAINT ON DB
	...
	try {
		await transactionQuery(async (client) => {
			// If like exists, the unique constraint will throw an error or do nothing.
			// We use 'ON CONFLICT DO NOTHING' to prevent an exception, or check rows affected.
			const insertLike = await client.query(`INSERT INTO likes (user_id, post_id) VALUES ($1, $2) ON CONFLICT (user_id, post_id) DO NOTHING RETURNING *`, [loggedInUserId, postId]);

			// If no row was returned, the user already liked it.
			if (insertLike.rowCount === 0) {
				throw new Error("ALREADY_LIKED");
			}
			// client.query(`INSERT INTO likes (user_id, post_id) VALUES ($1, $2);`, [loggedInUserId, postId]);
...
export async function removeLikeOnPost(req: Request, res: Response) {
	const loggedInUserId = req.user?.userId;
	const postId = req.params.id;

	try {
		const result = await transactionQuery(async (client) => {
			const deleteLike = await client.query(`DELETE FROM likes WHERE post_id=$1 AND user_id=$2 RETURNING *`, [postId, loggedInUserId]);
			if (deleteLike.rows.length === 0) {
				return { success: false, message: "No like found to be deleted" };
			}

			//The "like" was deleted, update the like_count in post
			await client.query(`UPDATE posts SET like_count = like_count - 1 WHERE _id=$1`, [postId]);
			return { success: true };
		});
