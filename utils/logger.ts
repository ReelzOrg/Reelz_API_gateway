...
const devLogger = pino({
	level: 'debug', // Set minimum log level to debug for development (trace levels will be suppressed)
	transport: {
		target: 'pino-pretty',
		options: {
			colorize: true,
...
const prodLogger = pino({
	level: 'info', // Set minimum log level to info for production
	formatters: {
		level: (label) => ({ level: label.toUpperCase() }),
	},
	timestamp: () => `,"time":"${new Date(Date.now()).toISOString()}"`,
 ...
 * @param moduleName - Name of the module or file
 ...
 * @returns A child logger instance with file context
 */
export const createFileLogger = (filePath: string) => {
