...
export default async function handleFileUpload(req: Request, res: Response, filePath: string | string[]) {
  ...
  const uploadedFiles: { s3Key: string | undefined, s3Location: string | undefined }[] = [];

  try {
    (req.files as Express.Multer.File[] | undefined)?.forEach(async (file: Express.Multer.File, index: number) => {
...
export function multerErrorHandler(err: Error, req: Request, res: Response, next: NextFunction) {
